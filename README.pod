...
domanent use for web frameworks was to hook a database (or similar data store)
...
the domanent use cases of the time.  Although the community eventually offered
some support for RESTful and AJAX use cases via non core addins, those suffered
from issues regarding how they handle content negotiation and since they were
project add ins, were not able to fully take advantage of deep integration.
...
will not be considered proper for public use.  Here's the proposed example:
    ...
    __PACKAGE__->config(
      "Catalyst::BodyDataParsers" => {
        'application/json' => sub { JSON::decode $_ }
      },
    );

We defined a new top level configuration key called C<Catalyst::BodyDataParsers>
...
A key may contain regular expressions since it will be matched against the
request content type in regular expression context.  This is to enable
situations where more than one content type is mapped to the same parser.  For
example:

    __PACKAGE__->config(
      "Catalyst::BodyDataParsers" => {
        'application/(?:vnd.+)?json' => sub { JSON::decode $_ }
      },
    );

Would allow you to match "application/vnd.mycompany.user+json", which is not an
uncommon approach to user defined content types.

A value must be a coderef, which is evaluated with C<$_> localized to the string
